...
# Create target variables for ML regression
# We'll predict the difference between home and away ML odds
ml_home = data['ML_Home']
ml_away = data['ML_Away']
ml_difference = ml_home - ml_away  # Positive means home team favored
...
print("Training XGBoost ML Difference Regression Model...")
print("=" * 50)

for x in tqdm(range(200)):
    x_train, x_test, y_train, y_test = train_test_split(data, ml_difference, test_size=.1, random_state=x)
    ...
    if mse == min(mse_results):
        model.save_model('../../Models/XGBoost_ML_Difference_{:.3f}_MSE.json'.format(mse))
