...
# Use OU (Over/Under line) as target variable for regression
ou_values = data['OU']
...
print("Training XGBoost OU Value Regression Model...")
print("=" * 50)

for x in tqdm(range(200)):
    x_train, x_test, y_train, y_test = train_test_split(data, ou_values, test_size=.1, random_state=x)
    ...
    if mse == min(mse_results):
        model.save_model('../../Models/XGBoost_OU_Value_{:.3f}_MSE.json'.format(mse))
