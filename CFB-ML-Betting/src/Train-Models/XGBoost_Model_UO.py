...
def train_xgboost_uo_model():
    """Train XGBoost model for Over/Under (UO) prediction using CFBD data."""
    print("Training XGBoost Over/Under Model...")
    ...
    try:
        ...
        # Prepare target variable (OU Cover: 0=Under, 1=Over, 2=Push)
        y = data['OU_Cover'].values
        
        # Prepare features - exclude non-numeric and target columns
        exclude_columns = [
            'Date', 'Home_Team', 'Away_Team', 'Home_Team_Win', 'Points', 
            'Win_Margin', 'OU_Cover', 'ML_Home', 'ML_Away'
        ]
        ...
        print(f"OU Cover classes: Under={np.sum(y==0)}, Over={np.sum(y==1)}, Push={np.sum(y==2)}")
        ...
        for x in tqdm(range(100)):  # Reduced from 300 for faster testing
            ...
            if accuracy > best_accuracy:
                ...
                model.save_model(f'../../Models/XGBoost_{accuracy*100:.1f}%_UO-9.json')
        ...
        if best_accuracy > 0:
            ...
            best_model.load_model(f'../../Models/XGBoost_{best_accuracy*100:.1f}%_UO-9.json')
