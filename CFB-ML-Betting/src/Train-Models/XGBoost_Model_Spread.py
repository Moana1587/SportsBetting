...
def train_xgboost_spread_model():
    """Train XGBoost model for spread prediction using CFBD data."""
    ...
    try:
        ...
        # Prepare target variable (Spread Cover: 1=Home covers, 0=Away covers)
        # Home team covers if they win by more than the spread
        y = ((data['Win_Margin'] + data['Spread']) > 0).astype(int).values
        ...
        print(f"Spread Cover classes: Away covers={np.sum(y==0)}, Home covers={np.sum(y==1)}")
        ...
        for x in tqdm(range(100)):  # Reduced from 300 for faster testing
            ...
            if accuracy > best_accuracy:
                ...
                model.save_model(f'../../Models/XGBoost_{accuracy*100:.1f}%_Spread-4.json')
        ...
        if best_accuracy > 0:
            ...
            best_model.load_model(f'../../Models/XGBoost_{best_accuracy*100:.1f}%_Spread-4.json')
